...
public class BoardBackup1611121620 {

	private static Point clickPoint = new Point();
	private static Coordinates tileSelected = null;
//	private static Coordinates lastTileSelected = null;
...
//		new Pawn(	new Coordinates('G', 1), chess.Color.player);
	...
	static void update(Graphics g) {
		...
		drawPieces(g);
		drawCoordinates(g);
		if (BoardBackup1611121620.tileSelected != null) {
			tileSelected.getPiece().drawRealOptions(g);
		}
	}
...
//						squareSize,
//						squareSize,
						getOffsetX()+getSquareSize()*(piece.getX()-1),
						getOffsetY()+getBoardSize()-getSquareSize()*(piece.getY()),
						getSquareSize(),
						getSquareSize(),
						new JPanel()
					);
